...
#MBSS 28.1
def defence_route_options(host, username, password):
    ssh_client, shell = connect_to_router(host, username, password)
    shell.send('system-view\n')
    time.sleep(1)
    shell.send('display current-configuration  | include rr\n')
    time.sleep(1)
    output = shell.recv(65536).decode()
    ssh_client.close()
...
#################
#MBSS 29.1
def defence_against_ip_spoofing(host, username, password):
    ssh_client, shell = connect_to_router(host, username, password)
    shell.send('system-view\n')
    time.sleep(1)
    shell.send('display current-configuration  | include acl\n')
    time.sleep(1)
    output = shell.recv(65536).decode()
    ssh_client.close()
    return 'packet-filter' in output

#MBSS 30.1
def data_transmission_security(host, username, password):
    ssh_client, shell = connect_to_router(host, username, password)
    shell.send('system-view\n')
    time.sleep(1)
    shell.send('display ipsec proposal\n')
    time.sleep(1)
    output = shell.recv(65536).decode()
    ssh_client.close()
    return 'ESP protocol: authentication SHA2-HMAC-256, encryption 256-AES' in output

#MBS 31.1
def ipv6_nd_security(host, username, password):
    ssh_client, shell = connect_to_router(host, username, password)
    shell.send('system-view\n')
    time.sleep(1)
    shell.send('display current-configuration | include nd snooping\n')
    time.sleep(1)
    output = shell.recv(65536).decode()
    ssh_client.close()
    return '' in output
...
#MBS 32.1
def rule_1_acl_3000(host, username, password):
    ssh_client, shell = connect_to_router(host, username, password)
    shell.send('system-view\n')
    time.sleep(1)
    shell.send('display acl 3000\n')
    time.sleep(1)
    output = shell.recv(65536).decode()
    ssh_client.close()
    return 'rule 1 permit icmp' in output

#MBS 33.1
def port_protect_group(host, username, password):
    ssh_client, shell = connect_to_router(host, username, password)
    shell.send('system-view\n')
    time.sleep(1)
    shell.send('display current-configuration | include protect-group\n')
    time.sleep(1)
    output = shell.recv(65536).decode()
    ssh_client.close()
    return 'protect-group' in output

#MBSS 34.1
def port_isolation(host, username, password):
    ssh_client, shell = connect_to_router(host, username, password)
    shell.send('system-view\n')
    time.sleep(1)
    shell.send('display current-configuration | include isolation\n')
    time.sleep(1)
    output = shell.recv(65536).decode()
    ssh_client.close()
    return '' in output
...
#MBSS 35.1
def port_security_mac(host, username, password):
    ssh_client, shell = connect_to_router(host, username, password)
    shell.send('system-view\n')
    time.sleep(1)
    shell.send('display current-configuration | include mac-limit maximum\n')
    time.sleep(1)
    output = shell.recv(65536).decode()
    ssh_client.close()
    return '' in output
...
print(f"Check Passed: Defence against Route Options is setup." if telnet_check else "Check Failed: Defence against Route options is not setup.")
...
print(f"Check Passed: Defence against IP Spoofing is setup (Needs manual check)." if telnet_check else "Check Failed: Defence against IP Spoofing is not setup  (Needs manual check).")
...
print(f"Check Passed: Data Transmission Security is setup." if telnet_check else "Check Failed: Data Transmission Security is not setup.")
...
print(f"Check Passed: IPv6 ND Security is setup." if telnet_check else "Check Failed: IPv6 ND Security is not setup.")
...
print(f"Check Passed: ACL 3000 Rule 1 permit is setup." if telnet_check else "Check Failed: ACL 3000 Rule 1 permit is not setup.")
...
print(f"Check Passed: Port Protection is setup." if telnet_check else "Check Failed: Port Protection is not setup.")
...
print(f"Check Passed: Port Isolation is setup." if telnet_check else "Check Failed: Port Isolation is not setup.")
...
print(f"Check Passed: Port Security via MAC Limiting is setup." if telnet_check else "Check Failed: Port Security via MAC Limiting is not setup.")
